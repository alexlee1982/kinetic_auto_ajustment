...
int CTRL_USR_Init(void)
{
  ...
  commandShmID=shmget((key_t)MOTION_COMMAND_KEY,sizeof(MotionCommandBuffer),IPC_CREAT|0666);
	if(commandShmID==-1)
	{
  ...
  if((long long int)commandShmem == -1)
	{
  ...
  statusShmID=shmget((key_t)MOTION_STATUS_KEY,sizeof(MotionStatus),IPC_CREAT|0666);
	if(statusShmID==-1)
	{
  ...
  if((long long int)emcmotStatus == -1)
	{
		perror("status shmat:");
		shmdt(emcmotStatus);
  ...
  configShmID=shmget((key_t)MOTION_CONFIG_KEY,sizeof(MotionConfig),IPC_CREAT|0666);
	if(configShmID==-1)
	{
  ...
  if((long long int)emcmotConfig == -1)
	{
		perror("config shmat:");
		shmdt(emcmotConfig);
  ...
  feedbackShmID=shmget((key_t)MOTION_FEEDBACK_KEY,sizeof(MotionFeedback),IPC_CREAT|0666);
	if(feedbackShmID==-1)
	{
  ...
  jointShmID=shmget((key_t)MOTION_JOINTS_KEY,((sizeof(MotionJointParameter))*MAX_JOINTS),IPC_CREAT|0666);
	if(jointShmID==-1)
	{
		perror("config shmget:");
		exit(1);
  ...
  debugShmID=shmget((key_t)MOTION_DEBUG_KEY,sizeof(MotionDebug),IPC_CREAT|0660);
	if(debugShmID==-1)
	{
		perror("debug shmget:");
		exit(1);
  ...
  if((long long int)emcmotDebug == -1)
	{
		perror("debug shmat:");
		shmdt(emcmotDebug);
