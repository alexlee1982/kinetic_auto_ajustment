...
int readInputPosition()
{
	...
	for(i=0;i<6;i++)
	{
		...
		{
			//joint->pos_fb = 360.0*(g_servo[i]->currentPosition-emcmotConfig->encodeOriginOffset[i])/emcmotConfig->servoPulsePerRound[i]/emcmotConfig->mechTransRatio[i]-joints[4].pos_cmd/emcmotConfig->coupleRate;
			joint->pos_fb=joint->pos_cmd;
		}
		else
		{
			//joint->pos_fb = 360.0*(g_servo[i]->currentPosition-emcmotConfig->encodeOriginOffset[i])/emcmotConfig->servoPulsePerRound[i]/emcmotConfig->mechTransRatio[i];
			joint->pos_fb=joint->pos_cmd;
		}
...
int writeOutPosition()
{
	...
	for (joint_num = 0; joint_num < 6; joint_num++)
	{
		...
		if(1==emcmotConfig->encoderType[joint_num]&&emcmotStatus->enble&&interpList->motionState==COMMAND_EXEC)
		{
			...
			if (joint_num == 2)
			{
				if(fabs(outPutPulse-lastPulse)>10000)
					//printf("gap=%\d outPutPulse= %ld,lastPulse=%ld \r\n",outPutPulse-lastPulse,outPutPulse,lastPulse);
				lastPulse=outPutPulse;
				
				//printf("joint->pos_cmd=%f ", joint->pos_cmd);
				//printf("outPutPlulse=%ld \r\n", outPutPulse);
			}
